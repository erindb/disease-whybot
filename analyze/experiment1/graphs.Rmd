...
result_negations = graph_structure %>%
  ...
  mutate(source = paste("-", source, sep="")) %>%
  mutate(link_type = "pR")
graph_structure = graph_structure %>% filter(!(link_type %in% c("nC", "nR"))) %>%
  rbind(negations) %>% rbind(cause_negations) %>% rbind(result_negations)
graph_structure = unique(graph_structure)
...
orig_node_types = c("D", "pC", "pR", "nC", "nR", "pCpC", "pCnC", "pCnR", "pRpR", "pRnC", "pRnR", "nCpC", "nCpR", "nCnC", "nRpC", "nRpR", "nRnR")
...
new_node_positions = data.frame(variable = node_types) %>%
  mutate(r = sapply(char(variable), function(v) {
    return((orig_node_positions %>% filter(variable==v))$r[1])
  }))
...
for (i in 1:nrow(edges_df)) {
  s = edges_df$source[i]
  t = edges_df$target[i]
  lt = edges_df$link_type[i]
  if (lt == "pC") {
    edges_df$source[i] = t
    edges_df$target[i] = s
    edges_df$link_type[i] = "pR"
  }
}
...
graph is defined by edges, so i just need to make the edges

```{r}
edges_df = do.call(rbind, lapply(unique(df$workerid), function(worker_to_graph) {
  w = df %>%
    filter(!(parser_feedback%in%c("impossible", "weird"))) %>%
    select(disease, completed, response, positive_response, variable, workerid) %>%
    filter(workerid==worker_to_graph) %>%
    filter(!is.na(response))
  responses = w$positive_response
...
master_graph = make_graph(edges, directed=F)

nodes = names(V(g))

pdf(file="graphs/master_graph.pdf")
plot(master_graph,
     vertex.size=1,
     vertex.label=NA,
     edge.color=link_colors,
...
length(unique((df %>% filter(parser_feedback==""))$positive_response))

df %>% filter(parser_feedback=="weird") %>% select(response)
df %>% filter(parser_feedback=="impossible") %>% select(response)
```
